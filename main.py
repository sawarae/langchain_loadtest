...
# Weaveの初期化
logger.info("Weave初期化")


# パフォーマンス測定用のヘルパー関数
def log_performance_metrics(
    endpoint_name, processing_time, success, error_type=None, user_id=None
):
    ...
    if success:
        logger.info(
            f"✅ {endpoint_name} - 処理時間: {processing_time:.2f}秒 - ユーザー: {user_id}"
        )
    else:
        logger.error(
            f"❌ {endpoint_name} - エラー: {error_type} - 処理時間: {processing_time:.2f}秒 - ユーザー: {user_id}"
        )
...
@weave.op()
async def astream_chat_with_rag(
...
質問に答える際は、上記のコンテキスト情報を活用してください。"""
        ...
        if "429" in error_msg or "rate limit" in error_msg.lower():
            log_performance_metrics(
                "azure_openai_rate_limit", 0, False, "rate_limit_error", user_id
            )

        yield f"エラーが発生しました: {error_msg}"


@app.post("/chat/stream/with-rag")
async def stream_chat_with_rag(request: ChatRequest):
